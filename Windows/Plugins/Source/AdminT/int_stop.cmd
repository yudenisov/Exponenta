rem net stop VisualSVNServer
rem net stop vdfssvc
rem net stop hMailServer
net stop named
rem net stop WowzaStreamingEngineManager430
rem net stop WowzaStreamingEngine430
net stop SQLSERVERAGENT
net stop SQLBrowser
net stop SQLWriter
net stop mssql$sqlexpress
rem net stop MSSQLSERVER
rem net stop postgresql-x64-9.6
rem net stop postgresql-x64-11
rem net stop IBG_gds_db
net stop W3SVC
net stop "FileZilla Server"
rem net stop ftpsvc
rem net stop Tftpd32_svc
rem net stop stunnel
rem net stop UnrealIRCd
rem net stop Apache2.4
rem net stop php_cgi_2000
rem net stop php_cgi_2001
rem net stop php_cgi_2002
rem net stop php_cgi_2003
rem net stop php_cgi_2004
rem net stop php_cgi_2005
rem net stop php_cgi_2006
rem net stop php_cgi_2007
rem net stop VMwareHostd
rem net stop nginx
net stop MySQL
rem net stop cygserver
net stop i2p
net stop tor
net stop TokenBroker
rem net stop FDResPub
rem net stop sshd
rem cscript c:\Elevation\asf_close.vbs //NoLogo