...
# cp @datadir@/@PACKAGE@/var/www/DATOS.server.grandkozel.ru/* $ServerRoot/

# Modify and copy User's site directive
repla=s/DATOS.server.grandkozel.ru/$Hacker_host2/g
echo $repla > repla.tmp
sed -f repla.tmp @datadir@/@PACKAGE@/init/etc/apache2/sites-available/zzz-user-default.conf > /etc/apache2/sites-available/$Hacker_host2.conf
cp -R @datadir@/@PACKAGE@/init/var/log/apache2/DATOS.server.grandkozel.ru-error.log /var/log/apache2/$Hacker_host2-error.log
cp -R @datadir@/@PACKAGE@/init/var/log/apache2/DATOS.server.grandkozel.ru-access.log /var/log/apache2/$Hacker_host2-access.log
#rm repla.tmp
