...
#
# Script Returns:
# * 2 if package not properly installed
# * 0 otherwise
#
##
apt-get install -y bash
apt-get install -y sed
if [ ! -f $Exponenta1/bin/.sources.list.init.mydiff ] & [ !  -f /etc/apt/sources.list ]
then
...
sed -f $Exponenta1/bin/.sources.list.init.mydiff /etc/apt/sources.list.origin > /etc/apt/sources.list
apt-get update -y
