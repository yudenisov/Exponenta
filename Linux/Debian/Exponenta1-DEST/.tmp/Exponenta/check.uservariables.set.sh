...
if [ $# -le 0 ] 
then
	echo  "USAGE:  check.uservariablesset.sh «variable»"
	exit -1
fi
...
if [ ! -e ~/.bashrc ]
then
	echo  "GENERAL ERROR. Configuration file .bashrc not found for current user"
	exit 2
fi
...
then
	echo "The User Variable \$"$1" is present"
	exit 0
fi
...
echo "The User Variable \$"$1" is absent"
exit 1
